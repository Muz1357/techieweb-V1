<?php include 'model/editProduct.php'; ?>
...
<body class="bg-gray-100">
    ...
    <div class="layout-container m-8">
        ...
        <div class="w-3/4 bg-white rounded-lg p-6 shadow-lg ml-6 flex flex-col">
            ...
            <form action="model/update.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="product_name" id="product_name" class="w-full p-3 border-2 border-gray-300 rounded-lg" value="<?php echo htmlspecialchars($product['name']); ?>" required>
                </div>

                <div class="mb-4">
                    <label for="product_description" class="block text-gray-700">Product Description</label>
                    <textarea name="product_description" id="product_description" rows="4" class="w-full p-3 border-2 border-gray-300 rounded-lg" required><?php echo htmlspecialchars($product['description']); ?></textarea>
                </div>

                <div class="mb-4">
                    <label for="product_price" class="block text-gray-700">Product Price ($)</label>
                    <input type="number" name="product_price" id="product_price" class="w-full p-3 border-2 border-gray-300 rounded-lg" value="<?php echo htmlspecialchars($product['price']); ?>" required>
                </div>

                <div class="mb-4">
                    <label for="product_image" class="block text-gray-700">Product Image</label>
                    <input type="file" name="product_image" id="product_image" class="w-full p-3 border-2 border-gray-300 rounded-lg" accept="image/*">
                    <img src="./<?php echo htmlspecialchars($product['image_url']); ?>" alt="Current Product Image" class="mt-4" width="150">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="product_category" id="product_category" class="w-full p-3 border-2 border-gray-300 rounded-lg" required>
                        <option value="electronics" <?php echo ($product['category'] == 'Earbuds' ? 'selected' : ''); ?>>Earbuds</option>
                        <option value="fashion" <?php echo ($product['category'] == 'headphones' ? 'selected' : ''); ?>>headphones</option>
                        <option value="home" <?php echo ($product['category'] == 'mobilephones' ? 'selected' : ''); ?>>mobilephones</option>
                        <option value="beauty" <?php echo ($product['category'] == 'laptops' ? 'selected' : 'laptops'); ?>>Beauty</option>
                    </select>
                </div>

                <div class="flex justify-end">
                    <button type="submit" class="bg-lblue text-white px-6 py-2 rounded-full hover:bg-blue-600 shadow">Update Product</button>
                </div>
            </form>
        </div>
    </div>
